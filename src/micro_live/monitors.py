...
class SlippageMonitor:
    ...
    def record(self, symbol: str, side: str, expected: float, actual: float, qty: float, strategy: str = "", spread: float = 0.0) -> SlippageRecord:
        bps = (actual - expected) / expected * 10000 if expected > 0 else 0
        if side == "buy":
            bps = abs(bps)
        rec = SlippageRecord(symbol=symbol, side=side, strategy_id=strategy, expected_price=expected, actual_fill_price=actual, quantity=qty, slippage_bps=bps, spread_at_submit=spread)
        self._records.append(rec)
        return rec
...
class StopExecutionAudit:
    ...
    def record(self, symbol: str, entry: float, target_stop: float, actual_fill: float, fees: float = 0.0, latency_ms: float = 0.0) -> StopExecutionRecord:
        ...
        rec = StopExecutionRecord(symbol=symbol, entry_price=entry, target_stop_price=target_stop, target_stop_pct=target_pct, actual_fill_price=actual_fill, actual_price_move_pct=actual_pct, slippage_pct=slip, fees=fees, net_realized_loss=abs(actual_pct) + fees, latency_ms=latency_ms)
        self._records.append(rec)
        return rec
    ...
    def worst_slippage_pct(self) -> float:
        ...
        return max(vals) if vals else 0.0
    def count(self) -> int: return len(self._records)
    def all_records(self) -> list[StopExecutionRecord]: return list(self._records)

...
class RealFeeService:
    ...
    def get_schedule(self, symbol: str = "default") -> RealFeeSchedule:
        if symbol not in self._schedule_cache:
            self._schedule_cache[symbol] = RealFeeSchedule(
                symbol=symbol,
                maker_fee=self.fallback_taker,
                taker_fee=self.fallback_taker,
                source="fallback",
    ...
    def update_from_exchange(
        self, symbol: str, maker: float, taker: float, source: str = "api"
    ) -> None:
        sched = RealFeeSchedule(
            ...
            taker_fee=taker,
            source=source,
            buy_fee_estimated=taker,
            sell_fee_estimated=taker,
